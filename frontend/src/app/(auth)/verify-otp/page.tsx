...
function VerifyOtpForm() {
  ...
  const channel = params.get('channel') || (identifier.includes('@') ? 'email' : 'phone');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await authAPI.verifyOtp({ identifier, otp_code: code, purpose });
      if (res.data.mrn) {
        setMrn(res.data.mrn);
  ...
  async function handleResend() {
    ...
    try {
      ...
      setError((err as { message?: string })?.message || 'Could not resend OTP');
    }
  }
  ...
  return (
    <div className="space-y-6 opacity-0 animate-fade-up">
      ...
      <div className="space-y-2">
        <h2 className="text-3xl font-bold tracking-tight">Verify your {channel === 'email' ? 'email' : 'phone'}</h2>
        <p className="text-sm text-muted-foreground">
          We&apos;ve sent a 6-digit code to <span className="font-medium text-foreground">{identifier || 'your account'}</span>
        </p>
      </div>
